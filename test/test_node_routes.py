...
def mock_node_service():
    with patch("src.core_api.routes.node_routes.NodeService", autospec=True) as mock:
        yield mock


def test_update_success(test_client: TestClient, mock_node_service: MagicMock):
    node = Node(node_id=uuid4(), job_slots=1, cpu_count=1, memory_allocated=10)
    mock_node_service.return_value.update_node.return_value = node

    node_updates = NodeUpdates(job_slots=1, cpu_count=1, memory_allocated=10)
