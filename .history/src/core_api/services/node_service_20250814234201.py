...
class NodeService:
    ...
    def set_job_slots(self, node_id: str, slots: int) -> Optional[Node]:
        try:
            ...
            # Validation
            if not isinstance(slots, int):  # type: ignore
            ...
            if updated:
                self.logger.info(f"Updated node {node_id} slots to {slots}")
            else:
                self.logger.warning(f"Node {node_id} doesn't exist") 
            return updated
            
        except ValidationError as node_service_error:
            error_msg = f"Invalid job slots value: {str(node_service_error)}"
