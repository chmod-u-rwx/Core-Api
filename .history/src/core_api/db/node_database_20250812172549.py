...
class NodeDataBase:
    ...
    def __init__(self, db_name: str = "node_database"):      
        ...
        self.client = get_mongo_client(DATABASE_URL, connectTimeoutMS=DATABASE_CONNECT_TIMEOUT)
    ...
    def read(self, node_id: str) -> Optional['Node']:
        try:
            node_data = self.nodes.find_one({"_id": node_id})
            if node_data:
    ...
    def update(self, node: 'Node', partial: bool = False) -> bool:
        try:
            ...
            if partial:
                ...
                result = self.nodes.update_one({"_id": objectId(node_id)}, update_data) 
    ...
    def count(self, filter: Optional[Dict] = None) -> int:
        try:
            ...
            self.logger.error(f"Failed to count nodes with filter {filter}: {pymongo_error}")
            raise RuntimeError(f"Error counting nodes with filter {filter}: {pymongo_error}")
    ...
    def _node_to_dict(self, node: 'Node') -> Dict[str, Any]:
        if hasattr(node, 'dict'):
            ...
            raise ValueError("Node must be a pydantic model, not a dict")

    def _dict_to_node(self, node_data: Dict[str, Any]) -> 'Node':
        if 'id' not in node_data:
            raise ValueError("Node data must contain 'id' field")
        if hasattr(Node, 'parse_obj'):
            return Node.model_validate(node_data)
        elif hasattr(Node, 'model_validate'):
            return Node.model_validate(node_data)
        else:
            return Node(**node_data)  # Fallback if model_validate are not available
    ...
    def close(self):
        if self.client:
            ...
            self.logger.warning("No MongoDB connection to close")
