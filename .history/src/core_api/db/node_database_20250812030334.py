...
class NodeDataBase:
    ...
    def read_all(self, filter: Optional[Dict]  = None, limit: int ) -> List['Node']:
        if filter is None:
            filter = {} 

        try:
            nodes_data = self.nodes.find()
            return [self._dict_to_node(node_data) for node_data in nodes_data]
        except PyMongoError as pymongo_error:
            self.logger.error(f"Failed to read all nodes: {pymongo_error}")
            raise RuntimeError(f"Error reading all nodes: {pymongo_error}")
