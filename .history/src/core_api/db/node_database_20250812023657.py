...
class NodeDataBase:
    ...
    def _initialize_collection(self):
        try: #create index on node_id for uniqueness and fast lookups
            ...
            self.logger.warning("Existing id")
        except PyMongoError as pymongo_error:
            self.logger.error(f"Failed to initialize node collection: {pymongo_error}")
            raise RuntimeError(f"Error with the database: {pymongo_error}")