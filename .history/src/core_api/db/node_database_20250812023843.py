...
class NodeDataBase:
    ...
    def create(self, node_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            result = self.nodes.insert_one(node_data)
            self.logger.info(f"Node created with ID: {result.inserted_id}")
            return {"id": str(result.inserted_id), **node_data}
        except DuplicateKeyError:
            self.logger.error("Node with this ID already exists.")
            raise ValueError("Node with this ID already exists.")
        except PyMongoError as pymongo_error:
            self.logger.error(f"Failed to create node: {pymongo_error}")
